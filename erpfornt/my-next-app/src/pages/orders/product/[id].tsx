...
import { orderListProduct } from "@/api/api"; // API 호출 함수 import
import { OrdersDto } from "@/api/datatype";

import { useRouter } from 'next/navigation'; // next/router가 아님!

interface OrdersProductPageProps {
    orders: OrdersDto[];
    error: string | null;
    id: string;
}

const OrdersProductPage = ({ orders, error, id }: OrdersProductPageProps) => {
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
    ...
    try {
        orders = await orderListProduct(Number(id)); // API 호출
    } catch (err:any) {
